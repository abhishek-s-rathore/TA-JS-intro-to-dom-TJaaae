...
allHrsArray.forEach((elem) => (elem.style.background = "antiquewhite"));
...
let box1Element = [...document.querySelector(".one").children];
for (let i = 0; i < box1Element.length; i++) {
  box1Element[i].remove();
}
...
let parentNode = box16.parentNode;
...
document.querySelector(".one").lastElementChild.style.color = "black";
...
[...document.querySelector(".archive").children].forEach((elem, ind) => {
  if (ind >= 1) {
    elem.toggleAttribute("class", "awesome-box");
  }
});
